...
class CDIssue(Document):
	...
	def populate_depends_on(self):
		...
		task_issue = frappe.get_doc("CD Task", self.task_issue)
		if self.task_issue and self.status == "Open":
			...
			if self.name not in [row.issue_id for row in task_issue.multi_issue]:
				task_issue.append(
					"multi_issue", {"doctype": "CD Assigment Issue", "issue_id": self.name}
				)
				task_issue.save()
	...
	def validate_status_taken(self):
		...
		if issue_personel is not None:
			...
			for emp_issue in result:
				arr_issue_personel.append(emp_issue)
