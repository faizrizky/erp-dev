...
class CDProject(Document):
	...
	def copy_from_template(self):
		...
		"""
		if self.project_template and not frappe.db.get_all("CD Task", dict(project=self.name), limit=1):
			...
			for task in template.tasks:
				template_task_details = frappe.get_doc("CD Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for template_task in template_tasks:
			...
			project_task = frappe.get_doc("CD Task", project_task.name)
	...
	def check_depends_on_value(self, template_task, project_task, project_tasks):
		if template_task.get("depends_on") and not project_task.get("depends_on"):
			for child_task in template_task.get("depends_on"):
				child_task_subject = frappe.db.get_value("CD Task", child_task.task, "subject")
	...
	def check_for_parent_tasks(self, template_task, project_task, project_tasks):
		if template_task.get("parent_task") and not project_task.get("parent_task"):
			parent_task_subject = frappe.db.get_value("CD Task", template_task.get("parent_task"), "subject")
			corresponding_project_task = list(
	...
	def update_percent_complete(self):
		...
		total = frappe.db.count("CD Task", dict(project=self.name))
		total_progress = frappe.db.count("CD Task", {"project": self.name, "status": ("!=", "Cancelled")})
		...
		if not total_progress:
			...
			if (self.percent_complete_method == "Task Completion" and total > 0) or (
				...
				completed = frappe.db.sql(
					"""select count(name) from `tabCD Task` where
			...
			if self.percent_complete_method == "Task Progress" and total_progress > 0:
				progress = frappe.db.sql(
					"""select sum(progress) from `tabCD Task` where
					project=%s""",
					self.name,
				)[0][0]
			...
			if self.percent_complete_method == "Task Weight" and total_progress > 0:
				weight_sum = frappe.db.sql(
					"""select sum(task_weight) from `tabCD Task` where
					project=%s""",
					self.name,
				)[0][0]
				weighted_progress = frappe.db.sql(
					"""select progress, task_weight from `tabCD Task` where
					project=%s""",
					self.name,
					as_dict=1,
				)
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			frappe.db.set_value("CD Project", new_name, "copied_from", new_name)
...
def get_cost_center_name(project):
	return frappe.db.get_value("CD Project", project, "cost_center")
...
def create_duplicate_project(prev_doc, project_name):
	...
	task_list = frappe.get_all("CD Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = frappe.get_doc("CD Task", task)
...
def send_project_update_email_to_users(project):
	doc = frappe.get_doc("CD Project", project)
...
def send_project_status_email_to_users():
	...
	for d in frappe.get_all("Project Update", {"date": yesterday, "sent": 0}):
		...
		project_doc = frappe.get_doc("CD Project", doc.project)
...
def update_project_sales_billing():
	...
	exists_query = (
		"(SELECT 1 from `tab{doctype}` where docstatus = 1 and project = `tabCD Project`.name)"
	)
	project_map = {}
	for project_details in frappe.db.sql(
		"""
			SELECT name, 1 as order_exists, null as invoice_exists from `tabCD Project` where
			exists {order_exists}
			union
			SELECT name, null as order_exists, 1 as invoice_exists from `tabCD Project` where
		...
		project = project_map.setdefault(
			project_details.name, frappe.get_doc("CD Project", project_details.name)
		)
...
def create_kanban_board_if_not_exists(project):
	...
	project = frappe.get_doc("CD Project", project)
	if not frappe.db.exists("Kanban Board", project.project_name):
		quick_kanban_board("CD Task", project.project_name, "status", project.name)
...
def set_project_status(project, status):
	...
	for task in frappe.get_all("CD Task", dict(project=project.name)):
		frappe.db.set_value("CD Task", task.name, "status", status)
