...
class CDTimesheets(Document):
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = frappe.get_doc("CD Task", data.task)
				task.update_time_and_costing()
				task.save()
				tasks.append(data.task)

			elif data.project and data.project not in projects:
				frappe.get_doc("CD Project", data.project).update_project()
	...
	def set_project(self, data):
		data.project = data.project or frappe.db.get_value("CD Task", data.task, "project")
	...
	def get_overlap_for(self, fieldname, args, value):
		timesheet = frappe.qb.DocType("CD Timesheets")
		timelog = frappe.qb.DocType("CD Timesheet Detail")
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	query = f"""
		...
		FROM `tabCD Timesheet Detail` tsd
			INNER JOIN `tabCD Timesheets` ts
			ON ts.name = tsd.parent
		WHERE
			tsd.parenttype = 'CD Timesheets'
			AND tsd.docstatus = 1
...
def get_timesheet_detail_rate(timelog, currency):
	timelog_detail = frappe.db.sql(
		...
		ts.currency as currency FROM `tabCD Timesheet Detail` tsd
...
def get_timesheet(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql(
		"""select distinct tsd.parent from `tabCD Timesheet Detail` tsd,
...
def get_events(start, end, filters=None):
	...
	return frappe.db.sql(
		"""select `tabCD Timesheet Detail`.name as name,
			`tabCD Timesheet Detail`.docstatus as status, `tabCD Timesheet Detail`.parent as parent,
			from_time as start_date, hours, activity_type,
			`tabCD Timesheet Detail`.project, to_time as end_date,
			CONCAT(`tabCD Timesheet Detail`.parent, ' (', ROUND(hours,2),' hrs)') as title
		from `tabCD Timesheet Detail`, `tabTimesheet`
		where `tabCD Timesheet Detail`.parent = `tabTimesheet`.name
			and `tabCD Timesheets`.docstatus < 2
...
def get_timesheets_list(
	...
	if customer:
		...
		timesheets = frappe.db.sql(
			...
			FROM `tabTimesheet` ts, `tabCD Timesheet Detail` tsd
