...
frappe.ui.form.on("Employee", {
	onload: function (frm) {
		frm.set_query("branch", function () {
			if (frappe.user_roles.join(",").toLowerCase().includes("content")) {
				return {
					options: "CD Department",
				};
			} else {
				return {
					options: "Branch",
...
frappe.ui.form.on("Employee", {
	onload: function (frm) {
		...
		var cd_Designation = "content";

		var sd_Designation = "software";
			...
			frm.set_query("reports_to", function () {
				return {
					filters: {
						...
						designation: ["like", "%" + cd_Designation + "%"],
						status: "Active",
						role_profile: "HOD Content Division",
					},
				};
			});
		} else if (sdRole) {
