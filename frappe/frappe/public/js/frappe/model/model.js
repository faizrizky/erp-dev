...
$.extend(frappe.model, {
  ...
  get_docinfo: function(doctype, name) {
    return (
      (frappe.model.docinfo[doctype] && frappe.model.docinfo[doctype][name]) ||
      null
    );
  },
  ...
  has_workflow: function(doctype) {
    return frappe.get_list("Workflow", { document_type: doctype, is_active: 1 })
      .length;
  },
  ...
  delete_doc: function(doctype, docname, callback) {
    ...
    if (title_field)
      title = frappe.model.get_value(doctype, docname, title_field);
