...
frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
    ...
    if (doc) {
      if (typeof expression === "boolean") {
        ...
        try {
          ...
          frappe.throw(
            __('Invalid "depends_on" expression set in filter {0}', [
              filter_label,
            ])
          );
        }
  ...
  open_create_chart_dialog() {
    ...
    function preview_chart() {
      ...
      if (values.x_field && values.y_fields.length) {
        ...
        wrapper
          .find(".chart-container .title, .chart-container .sub-title")
          .hide();
        wrapper.show();
  ...
  pdf_report(print_settings) {
    ...
    print_settings.report_name = `${__(this.report_name)}_${filter_values.join(
      "_"
    )}.pdf`;
  ...
  get_filters_html_for_print() {
    ...
    return Object.keys(applied_filters)
      .map((fieldname) => {
        ...
        const custom_values_after = frappe.query_report.get_filter(fieldname).df
          .custom_values_after;
